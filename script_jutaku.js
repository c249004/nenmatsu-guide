async function loadHints() {
  // 🔹 jutaku_hints.json からガイド情報を取得
  const response = await fetch('jutaku_hints.json');
  ...
  function typeText(element, text, speed = 30) {
    ...
    typingInterval = setInterval(() => {
      ...
      if (i > text.length) {
        ...
        // 🌼 タイピング終了後、guide-note の透明度を戻す
        if (note) note.style.opacity = '1.0';
      }
    }, speed);
  }

  // 🔹 jutaku_hints.json にある各ヒント領域を順番に処理
  hints.forEach(item => {
